...
function themeColor(e) {
    ...
    document.querySelectorAll(".theme-img , .theme-color").forEach(function(r) {
        ...
        n && (n = n.id, window.localStorage.setItem("image", n), i = document.getElementsByClassName(n), e && (l = window.getComputedStyle(i[0], null).getPropertyValue("background-image"), document.querySelector(".user-chat").style.backgroundImage = l)), r.addEventListener("click", function(e) {
            r.id == c && (r.checked = !0), r.id == d && (r.checked = !0);
    ...
    function o() {
        "users" == m ? (document.getElementById("channel-chat").style.display = "none", document.getElementById("users-chat").style.display = "block") : (document.getElementById("channel-chat").style.display = "block", document.getElementById("users-chat").style.display = "none"), _(s + "chats.json")
    }
    e("users.json", function(e, t) {
        null !== e ? console.log("Something went wrong: " + e) : (t[0].favorites.forEach(function(e, t) {
            var a = e.profile ? '<img src="' + e.profile + '" class="rounded-circle avatar-xs" alt=""><span class="user-status"></span>' : '<div class="avatar-xs"><span class="avatar-title rounded-circle bg-primary text-white"><span class="username">JP</span><span class="user-status"></span></span></div>',
                ...
                l = 1 === e.id ? "active" : "";
            document.getElementById("favourite-users").innerHTML += '<li id="contact-id-' + e.id + '" data-name="favorite" class="' + l + '">                  ' + i + '                       <div class="d-flex align-items-center">                          <div class="chat-user-img online align-self-center me-2 ms-0">                              ' + a + '                          </div>                          <div class="overflow-hidden">                              <p class="text-truncate mb-0">' + e.name + "</p>                          </div>                          " + s + "                      </div>                  </a>              </li>"
            ...
            l.addEventListener("click", function(e) {
                ...
                document.getElementById("users-conversation").querySelectorAll(".left .chat-avatar").forEach(function(e) {
                    i ? e.querySelector("img").setAttribute("src", i) : e.querySelector("img").setAttribute("src", n)
                }), window.stop()
            })
        ...
        var i, l;
        null !== e ? console.log("Something went wrong: " + e) : ((f = t).sort(function(e, t) {
            return e.name.localeCompare(t.name)
        }), l = i = "", f.forEach(function(e, t) {
    ...
    document.querySelector("#audiofile-input").addEventListener("change", function() {
        ...
        e.readAsDataURL(r), a && a.classList.add("show"), e.addEventListener("load", function() {
            var e = r.name,
                t = Math.round(r.size / 1e6).toFixed(2);
            a.innerHTML = '<div class="card p-2 border mb-2 audiofile_pre d-inline-block position-relative">            <div class="d-flex align-items-center">                <div class="flex-shrink-0 avatar-xs ms-1 me-3">                    <div class="avatar-title bg-soft-primary text-primary rounded-circle">                        <i class="bx bx-headphone"></i>                    </div>                </div>                <div class="flex-grow-1 overflow-hidden">                <h5 class="font-size-14 text-truncate mb-1">' + e + '</h5>                  <input type="hidden" name="downloadaudiodata" value="' + r + '"/>                        <p class="text-muted text-truncate font-size-13 mb-0">' + t + 'mb</p>                </div>                <div class="flex-shrink-0 ms-3">                    <div class="d-flex gap-2">                        <div>                        <i class="ri-close-line text-danger audioFile-remove"  id="remove-audioFile"></i>                        </div>                    </div>                </div>            </div>          </div>', b = e, x = t, removeAudioFile(), S[E] = r
        }, !1), E++
    });
    var q, k, c, L = [],
        A = 1;
